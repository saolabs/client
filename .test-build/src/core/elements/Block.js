...
class Block {
    constructor({ ctx, name, viewId = null, contentRenderFactory = (parentElement) => [], id = null, initMode = 'create' }) {
        ...
        if (this.initMode === common_1.InitModes.HYDRATE) {
            let marker = MarkerService_1.SaoMarker.first('block', this.id);
            if (marker) {
                this.marker = marker;
                ...
                this.openTag = MarkerRegistry_1.default.createMarkerStart('block', this.id);
                this.closeTag = MarkerRegistry_1.default.createMarkerEnd('block', this.id);
                MarkerRegistry_1.default.register('block', this.id, { name, viewId }); // Register block in marker registry
