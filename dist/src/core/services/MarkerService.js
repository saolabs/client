...
export class MarkerService {
    ...
    query(tagOrShortcut, where, useCache = true) {
        ...
        for (let i = 0; i < commentNodes.length; i++) {
            ...
            const openMatch = commentText.match(new RegExp(`^${this.prefix}${this.delimiter}([^${this.delimiter}]+)${this.delimiter}(.+)-s$`));
            if (openMatch) {
            ...
            const closeMatch = commentText.match(new RegExp(`^${this.prefix}${this.delimiter}([^${this.delimiter}]+)${this.delimiter}(.+)-e$`));
            if (closeMatch) {
