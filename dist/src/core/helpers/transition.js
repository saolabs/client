...
const leavingNodes = new WeakSet();
...
const isCurrent = (el, token) => generation.get(el) === token;
...
export async function runEnter(el, name) {
    const token = bump(el);
 ...
 */
export async function runLeave(el, name) {
    const token = bump(el);
