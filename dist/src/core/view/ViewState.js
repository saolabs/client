...
export class StateManager {
    constructor(stateInstance, controller) {
        ...
        this.computedDependents = new Map();
    ...
    invalidateComputed(key, notify = true) {
    ...
    computed(key, fn, deps = []) {
        if (this._isDestroyed)
            throw new Error('[ViewState] Cannot register computed after destroy.');
        ...
        if (deps.some(dep => reaches(dep)))
            throw new Error(`[ViewState] Computed dependency cycle: ${key}`);
        const existing = this.states[key];
        if (existing && !existing.__computed__)
            throw new Error(`[ViewState] State already exists: ${key}`);
        for (const dep of this.computedNodes.get(key)?.deps ?? []) {
            const dependents = this.computedDependents.get(dep);
            dependents?.delete(key);
            if (dependents?.size === 0)
                this.computedDependents.delete(dep);
        }
        let cache;
        let dirty = true;
        ...
        Object.defineProperty(slot, 'value', {
            get: () => {
                if (dirty) {
                    if (evaluating)
                        throw new Error(`[ViewState] Recursive computed read: ${key}`);
                    evaluating = true;
                    try {
                        cache = fn();
                        dirty = false;
        ...
        for (const dep of deps) {
            if (!this.computedDependents.has(dep))
                this.computedDependents.set(dep, new Set());
    ...
    untrackAllArrays() {
        for (const key of Array.from(this.trackedChannels.keys()))
            this.untrackKey(key);
     ...
     */
    static observe(node, channel, seen) {
        if (!StateManager.isObservable(node) || seen.has(node))
            return;
        seen.add(node);
        let hooks = node[StateManager.HOOKS];
        const fresh = !hooks;
        if (!hooks) {
            hooks = new Set();
            try {
        ...
        const observeIncoming = (next) => {
            if (!StateManager.isObservable(next))
                return; // primitive: thoát sớm, không cấp phát
            const set = node[StateManager.HOOKS];
            if (!set)
                return;
        ...
        if (Array.isArray(node)) {
            ...
            for (const item of node)
                StateManager.observe(item, channel, seen);
            return;
        }
        for (const prop of Object.keys(node)) {
            StateManager.observe(node[prop], channel, seen);
            if (!fresh)
                continue; // accessor đã cài từ lần trước
