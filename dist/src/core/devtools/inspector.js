...
class Inspector {
    ...
    // ── Vẽ nội dung ───────────────────────────────────────────────
    refresh() {
        const body = this.panel?.querySelector('[data-role="body"]');
        if (!body)
            return;
        ...
        if (tree.length === 0) {
            ...
            for (const node of tree)
                body.append(this.renderNode(node, 0));
        }
        const log = devtools.getLog().slice(-25).reverse();
        body.append(this.section(`Sự kiện gần đây (${log.length})`));
        if (log.length === 0)
            body.append(this.muted('Chưa có sự kiện.'));
        else
            for (const e of log)
                body.append(this.renderEvent(e));
    }
    muted(text) {
    ...
    renderNode(node, depth) {
        ...
        for (const child of node.children)
            wrap.append(this.renderNode(child, depth + 1));
        return wrap;
    }
    renderEvent(e) {
        const colors = {
    ...
    describe(e) {
        if (e.type === 'state:changed')
            return `${e.path ?? ''} [${(e.detail?.keys ?? []).join(', ')}]`;
        if (e.type === 'error')
            return `${e.detail?.phase ?? ''} ${e.path ?? ''}: ${e.detail?.message ?? ''}`;
        return e.path ?? '';
    }
...
export function bindDevtoolsShortcut() {
