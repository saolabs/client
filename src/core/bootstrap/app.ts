 ...
 */
function drainPushQueue(): void {
    if (typeof window === 'undefined') return;
    const w = window as any;

    const pending: unknown[] = Array.isArray(w.App) ? w.App : [];

    (App as any).push = (fn: (app: ApplicationInterface) => void): void => {
    ...
    for (const fn of pending) (App as any).push(fn);
}
 ...
 */
App.setMethod('start', async function (config?: Record<string, any>) {
    if (App.isStarted) {
        console.warn('[Bootstrap] App already started.');
        return;
    }
    let bundles: MergedBundles | null = null;
    if (!App.isInitialized) {
